...
public class Obj3D
{
    ...
    private boolean readObject(Input inp)
    {
        for (;;)
        {
            ...
            if (i < 0)
            {
                String message = "Número de vértice negativo en la primera parte del archivo de entrada";
                JOptionPane.showMessageDialog(null,message,"Error al leer archivo",JOptionPane.ERROR_MESSAGE);
                return failing();
            }
        ...
        if (count < 6 || count > 8)
        {
            String message = "Archivo de entrada inválido";
            JOptionPane.showMessageDialog(null,message,"Error al abrir archivo",JOptionPane.ERROR_MESSAGE);
            return failing();
        }
        for (;;)
        {
            ...
            for (;;)
            {
                ...
                if (i == 0 || absi >= w.size() || w.elementAt(absi) == null){
                    String message = "Declaración inválida: el vértice " + absi +
                            " debe definirse, siendo distinto de cero y menor a " + w.size();
                    JOptionPane.showMessageDialog(null,message,"Error al leer archivo",JOptionPane.ERROR_MESSAGE);
                    return failing();
                }
   ...
   private void initPersp()
   {
    ...
    {
        double inprod = a * sunX + b * sunY + c * sunZ;
        return (int)(((inprod - inprodMin)/inprodRange) * 255);
    }
}
